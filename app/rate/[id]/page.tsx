...
import { readCachedAlbumDetail, writeCachedAlbumDetail } from "@/lib/album-cache";
...
export default async function RatePage({ params }: Props) {
  let album: Awaited<ReturnType<typeof getSpotifyAlbum>> | null = null;
  const cached = await readCachedAlbumDetail(params.id);
  ...
  try {
    if (!album) {
      album = await getSpotifyAlbum(params.id);
      await writeCachedAlbumDetail(params.id, album);
    }
  } catch (error) {
    if (cached) {
      album = cached.album;
    ...
    <main className="vibe-glow-bg min-h-screen">
      <div className="mx-auto w-[min(1200px,92vw)] py-8">
        <div className="mb-6 flex items-center gap-3">
          ...
          <Link
            href={primaryArtist ? `/artist/${primaryArtist.id}` : "/"}
